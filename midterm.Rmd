...
contours <- st_read("C:/Users/Kyle McCarthy/Documents/CPLN 675/Midterm/Contours/contours.shp")%>% 
  st_transform(crs = sr)%>% 
  st_make_valid()
...
inundation_Fish <- 
  inundation_Fish%>% 
  mutate(Park_nn1 = nn_function(st_c(st_centroid(.)), st_c(parks), 10))
...
ParkStats<- 
  ...
  na.omit()%>%
  group_by(Label)%>% 
  summarise( ParkAvgDist = mean(Park_nn1))



ggplot()+ 
  geom_bar(data = ParkStats, aes(x = Label, y = ParkAvgDist), stat="identity", fill = "blue")
